...
import fantaTeamContext from "../context/fantaTeam/context";
...
function CreateFantaTeam() {
	const { createFantaTeam, getFantaTeam } = useContext(fantaTeamContext);
	...
	const selectedDrivers = useMemo(
		() => drivers.filter((driver) => selectedDriversIDs.includes(driver.id)),
		[selectedDriversIDs, drivers]
	);

	const selectedTeams = useMemo(
		() => teams.filter((team) => selectedTeamsIDs.includes(team.id)),
	...
	const toggleSelectedDriver = useCallback(
		(id: number) => {
		...
		[budgetAllowance, currentBudget, drivers, selectedDriversIDs]
	);

	const toggleSelectedTeam = useCallback(
		(id: number) => {
		...
		[budgetAllowance, currentBudget, selectedTeamsIDs, teams]
	);

	const sendFormation = useCallback(() => {
	...
	const toggleOpenBottomSheet = (bool = !isBottomSheetOpen) => {
		setIsBottomSheetOpen(bool);
	};
	useEffect(() => {
		const getFormation = async () => {
			const fantaTeam = await getFantaTeam();
			console.log(fantaTeam);
		};
		getFormation();
	}, []);
	...
	}, [selectedDriversIDs.length, selectedTeamsIDs.length]);
			...
			<div className="create-formation-page">
						...
						<tbody>
							{drivers.map(
								({ id, firstname, lastname, team, cost }: DriverType) => (
									...
									`}
										key={id}
										onClick={() => toggleSelectedDriver(id)}
									>
				...
				>
					...
					<Button
						...
						onClick={() => {
							createFantaTeam({ selectedTeams, selectedDrivers });
						}}
					>
						Salva Formazione
					</Button>
...
export default CreateFantaTeam;
